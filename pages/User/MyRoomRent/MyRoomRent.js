...
Page({
  ...
  bindinput1: function (e) {
    var roomDetail = wx.getStorageSync('roomDetail')
    roomDetail.title = e.detail.value
    var value = e.detail.value + "";
    this.setData({ title_length: value.length });
    wx.setStorageSync('roomDetail', roomDetail)
  },
  bindinput2: function (e) {
    var roomDetail = wx.getStorageSync('roomDetail')
    roomDetail.desc = e.detail.value
    var value = e.detail.value + "";
  ...
  bindinput3: function (e) {
    var roomDetail = wx.getStorageSync('roomDetail')
    roomDetail.reason = e.detail.value
    var value = e.detail.value + "";
    this.setData({ reason_length: value.length });
    wx.setStorageSync('roomDetail', roomDetail)
  },
  bindinput4: function (e) {
    var roomDetail = wx.getStorageSync('roomDetail')
    roomDetail.require = e.detail.value
    var value = e.detail.value + "";
    this.setData({ require_length: value.length });
    wx.setStorageSync('roomDetail', roomDetail)
  },
  bindinput5: function (e) {
    var roomDetail = wx.getStorageSync('roomDetail')
    roomDetail.think_talk = e.detail.value
    var value = e.detail.value + "";
    this.setData({ think_talk_length: value.length });
    wx.setStorageSync('roomDetail', roomDetail)
  },
  ...
  formSubmit: function (e) {
    ...
    if (!that.data.cover_id) {
      util.alertShow(this, "cancel", "必须设置某一张爱家创想图为封面");
      return;
    }
    if (isNaN(data.floor)) {
      util.alertShow(this, "cancel", "楼层必须是一个数字");
      return;
    }
