...
Page({
  ...
  loadData(option){
    ...
    var params = {
      ...
      types:option,
      p: that.data.activeIndex == 0 ? that.data.p : 1,
    };
    ...
    util.getMessages(params, function (res) {
      ...
      if(res.code == 200){
        ...
        that.setData({
          items: msgList,
          p: that.data.activeIndex == 0 ? parseInt(that.data.p) + 1 : 1
        });
        if (data.length == 0) {
          wx.showToast({
            title: '无更多数据.',
  ...
  clickToDetail:function(e){
    ...
    if(type == 2){//申请理想之家
     ...
     url = '/pages/Home/Apply?id='+id+'&applyuid=' + sendUid 
      ...
      url = "/pages/User/MyLdealHome/LdealHome/LdealHome?id="+rel_id+"&uid="+homeuid
