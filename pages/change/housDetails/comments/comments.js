...
Page({
  ...
  addComment: function (that, e) {
    ...
    if (!that.data.replayUser.startsWith("@")) { //留言
      paramMsg = {
        ...
        page: 'pages/change/housDetails/comments/comments?type=' + that.data.type + '&home_id=' + that.data.room_id + '&homeuid=' + that.data.home_uid + "&title=" + that.data.title,
      };
    } else {//回复
  ...
  toHomeDetail: function (e) {
    ...
    var homeuid = data.homeuid;
    wx.redirectTo({
      url: '/pages/change/housDetails/housDetails?id=' + homeid + "&uid=" + homeuid,
    })
  },
  ...
  onLoad: function (options) {
    ...
    wx.setNavigationBarTitle({
      title: options.title + "-" + (options.type == 1 ? '留言' : '点子'),
    });

    //加载数据
    that.setData({
      home_id: options.room_id,
      home_uid: options.roomuid,
      type: options.type,
      title: options.title,
